...
(async () => {
  ...
  try {
    ...
    await cdc.locator('[data-action="next"]').click();
    assert.equal(await cdc.locator('.flow-player__node-summary').textContent(), 'Source database where committed changes originate.');
    assert.match(await cdc.locator('.flow-player__node-doc a').getAttribute('href'), /concepts\/cdc\.html$/);
    await cdc.locator('.flow-player__scenario-select').selectOption('1');
    await page.waitForFunction(() => document.querySelector('[data-flow-id="target-offline"]'));
    ...
    await offline.locator('.flow-player__scenario-select').selectOption('0');
    await page.waitForFunction(() => document.querySelector('[data-flow-id="normal-replication"]'));
    ...
    await darkPage.waitForFunction(() => document.querySelector('[data-flow-id="target-offline"]'));
